...
TWILIO_NUMBER = os.getenv("TWILIO_NUMBER", "+10000000000")
...
# ---------------- VIDEO STREAMING (for dashboard) ----------------
STREAM_PORT = 5001

app_flask = Flask(__name__)
latest_frames = {}          # cam_id -> latest JPEG bytes
...
def analyze_frame(frame, label="uploaded"):
    """Run YOLO people-detection + draw overlays on a single standalone image.
    Reuses the same detection/NMS logic as the live camera pipeline."""
    ...
    capacity = max(people, 1)   # simple estimate for a single still image
    ...
    return frame, people, capacity, density


@app_flask.route("/process_image", methods=["POST"])
def process_image():
    ...
    processed, people, capacity, density = analyze_frame(frame)

    ok, jpeg = cv2.imencode(".jpg", processed, [cv2.IMWRITE_JPEG_QUALITY, 85])
    if not ok:
        return {"error": "Failed to encode result image"}, 500
...
def run_stream_server():
    app_flask.run(host="0.0.0.0", port=STREAM_PORT, threaded=True)
...
class CrowdCamera:
    ...
    def process(self, thresholds_cfg):
        ...
        # publish frame for the web dashboard's MJPEG stream
        ok, jpeg = cv2.imencode(".jpg", frame, [cv2.IMWRITE_JPEG_QUALITY, 80])
        if ok:
            with frame_lock:
                latest_frames[self.cam_id] = jpeg.tobytes()
